...
Public Class frmCrossSellingCityConfig
    ...
    Private Sub frmCrossSellingCityConfig_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        Me.cbbCity.Select()
        blSkipDeal = True
    ...
    Private Sub frmCrossSellingCityConfig_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub frmCrossSellingCityConfig_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        dtCityRule = DB.GetDataTable("Select * From CrossSellingCityRule Where CityID=" & dtCity.Rows(0)("AreaID").ToString & " And RuleState<>3")
        ...
        Try
            Dim dvCityRuleDetails As DataView = DB.GetDataTable("Select * From CrossSellingCityRuleDetails Where CityID=" & dtCity.Rows(0)("AreaID").ToString & " And RuleState<>3").DefaultView
        ...
        blCanModify = (frmMain.dtAllowedRight.Select("RightName='CrossSellingCityConfigModify'").Length > 0)
        blCanValidate = (frmMain.dtAllowedRight.Select("RightName='CrossSellingCityConfigValidate'").Length > 0)
    ...
    Private Sub cbbCity_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbbCity.SelectedIndexChanged
        ...
        If drCity("IsLoaded").ToString <> "True" Then
            ...
            Else
                dtRuleTemp = DB.GetDataTable("Select * From CrossSellingCityRule Where CityID=" & sCityID & " And RuleState<>3")
                If dtRuleTemp Is Nothing Then
                    frmMain.statusText.Text = "系统因在检索数据时出错而无法检索城市设置。请联系软件开发人员。"
                Else
                    Try
                        Dim dvCityRuleDetails As DataView = DB.GetDataTable("Select * From CrossSellingCityRuleDetails Where CityID=" & sCityID & " And RuleState<>3").DefaultView
                        dvCityRuleDetails.RowFilter = "RuleState=2"
    ...
    Private Function SaveChanges() As Boolean
            ...
            If drRuleRows.Length = 0 Then
                ...
                If drRuleRows.Length = 0 Then
                    ...
                    If drRuleRows.Length > 0 Then
                        ...
                        drChange("SQL") = "Exec CrossSellingCityRebateRuleModification @RuleID=" & drRuleRows(0)("RuleID", DataRowVersion.Original).ToString & ",@IsDeleted=1,@SSID=" & frmMain.sSSID
                    End If
                Else
                    ...
                    If drDetailRows.Length = 0 Then
                        If drRuleRows(0)("RuleState") <> drRuleRows(0)("RuleState", DataRowVersion.Original) Then
                            ...
                            If drRuleRows(0)("RuleState") = 1 Then
                                ...
                                drChange("SQL") = "Exec CrossSellingCityRebateRuleModification @RuleID=" & drRuleRows(0)("RuleID").ToString & ",@RuleState=1" & IIf(drRuleRows(0)("StateReason").ToString = "", "", ",@StateReason='" & drRuleRows(0)("StateReason").ToString.Replace("'", "''") & "'") & ",@SSID=" & frmMain.sSSID
                            Else
                                ...
                                drChange("SQL") = "Exec CrossSellingCityRebateRuleModification @RuleID=" & drRuleRows(0)("RuleID").ToString & ",@CityID=" & drCity("AreaID").ToString & ",@RuleState=2,@SSID=" & frmMain.sSSID
                            End If
                            drChange.EndEdit()
                        End If
                    Else
                        ...
                        If drRuleRows(0)("RuleState") = drRuleRows(0)("RuleState", DataRowVersion.Original) OrElse drRuleRows(0)("RuleState", DataRowVersion.Original) = 1 Then
                            ...
                            drChange("SQL") = "Exec CrossSellingCityRebateRuleModification @CityID=" & drCity("AreaID").ToString & ",@SSID=" & frmMain.sSSID
                        Else
                            drChange("Description") = "修改并审核城市返点规则" & Chr(13) & "Modify and then approve City's Discount Scale."
                            drChange("Result") = "规则审核后立即生效" & Chr(13) & "Rule effective immediately since it had been approved."
                            drChange("SQL") = "Exec CrossSellingCityRebateRuleModification @CityID=" & drCity("AreaID").ToString & ",@RuleState=2,@SSID=" & frmMain.sSSID
                        End If
                        drChange.EndEdit()
                    End If
                End If
        ...
        If dtChanges.Select("SQL Like '%@RuleID=%'").Length <> dtChanges.Rows.Count Then '当参数中不包含@RuleID时表示需要修改RuleDetails
            If DB.ModifyTable("Select RowID,StartSalesAMT,EndSalesAMT,MaxRebateRate,ApprovableRoleID Into #tempDetails From CrossSellingCityRebateRuleDetails Where 1=2") = -1 Then
