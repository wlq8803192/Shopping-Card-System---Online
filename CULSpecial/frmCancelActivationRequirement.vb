Public Class frmCancelActivationRequirement
    ...
    Private Sub frmCancelActivationRequirement_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
    ...
    Private Sub frmCancelActivationRequirement_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
    ...
    Private Sub frmCancelActivationRequirement_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'modify code 047:start-------------------------------------------------------------------------
        If isSignCard Then
            Me.dgvList1("StartCardNo", 0).Value = signCardNo
            dgvList1_CellLeave(Nothing, Nothing)
            Me.dgvList1.Enabled = False
        End If
        'modify code 047:end-------------------------------------------------------------------------
    End Sub
    ...
    Private Sub dgvList1_CellValueChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvList1.CellValueChanged
        ...
        Select Case Me.dgvList1.Columns(e.ColumnIndex).Name
            Case "StartCardNo"
                ...
                ElseIf sCardNo.IndexOf("8") <> 0 AndAlso sCardNo.IndexOf("9") <> 0 AndAlso sCardNo.IndexOf("2336") <> 0 Then
                    sErrorInfo = "开始卡号错误：卡号非法（应以""2336""、""8""或""9""开头）！"
            ...
            Case "EndCardNo"
                ...
                ElseIf sCardNo.IndexOf("8") <> 0 AndAlso sCardNo.IndexOf("9") <> 0 AndAlso sCardNo.IndexOf("2336") <> 0 Then
                    sErrorInfo = "结束卡号错误：卡号非法（应以""2336""、""8""或""9""开头）！"
                ElseIf (sCardNo.IndexOf("2") = 0 AndAlso dvCULParameter.Table.Select("CULCardBin='" & sCardNo.Substring(4, 5) & "'").Length = 0) OrElse (sCardNo.IndexOf("2") <> 0 AndAlso dvCULParameter.Table.Select("CULCardBin='" & sCardNo.Substring(0, 5) & "'").Length = 0) Then
        ...
        If sErrorInfo = "" Then
            ...
            Else
                Me.lblInfo1.Text = "共输入了 " & Format(iCards, "#,0") & " 张卡。如果卡号输入完毕，请按""下一步""查询该" & IIf(iCards = 1, "张", "批") & "购物卡的状态及其对应的销售单。"
                blStep1OK = True
    ...
    Private Sub chkSelectAll_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkSelectAll.Click
        ...
        If iSelectedCards = 0 Then
            Me.lblInfo3.Text = "请选择需要进行激活撤销操作的购物卡对应的销售单号，然后按""下一步""输入撤销原因……"
            blStep3OK = False
            Me.btnNext.Enabled = False
        Else
        ...
        Select Case bStep
            Case 1
                ...
                ElseIf dtList2.Rows.Count = 0 Then
                    ...
                    Me.lblInfo2.Text = "正在查询该" & IIf(iCards = 1, "张", "批") & "购物卡的状态及其对应的销售单，请稍候……"
                    Me.Refresh()
                    ...
                    If Not DB.IsConnected Then
                        ...
                        Me.lblInfo2.ForeColor = Color.Red
                        Me.lblInfo2.Text = "系统因连接不到数据库而无法购物卡对应的销售单。请检查数据库连接。"
                    ...
                    If DB.ModifyTable("Select CardNo Into #tempCard From CULCancelActivation Where 1=2") = -1 OrElse DB.BulkCopyTable("#tempCard", dtTemp.DefaultView.ToTable(True, "CardNo")) = -1 Then
                        ...
                        dtResult.Dispose() : dtResult = Nothing
                        Me.lblInfo2.ForeColor = Color.Red
                        Me.lblInfo2.Text = "检查购物卡对应的销售单失败！"
                        Me.btnNext.Enabled = False
                        Me.btnPrevious.Select()
                        DB.Close() : Me.Cursor = Cursors.Default : Return
                    End If

                    'modify code 058:start-------------------------------------------------------------------------
                    'dtResult = DB.GetDataTable("Exec GetSalesBillInfo " & frmMain.sLoginUserID)
                    dtResult = DB.GetDataTable("Exec GetSalesBillInfo_CrossSelling " & frmMain.sLoginUserID)
                    'modify code 058:end-------------------------------------------------------------------------
                    DB.Close()
                    ...
                    If dtResult Is Nothing Then
                        dtTemp.Dispose() : dtTemp = Nothing
                        Me.lblInfo2.ForeColor = Color.Red
                        Me.lblInfo2.Text = "检查购物卡对应的销售单时出现数据库内部错误！"
                        Me.btnNext.Enabled = False
                        Me.btnPrevious.Select()
                        Me.Cursor = Cursors.Default : Return
                    End If
                    ...
                    For Each drCUL As DataRow In dtTemp.Rows
                        ...
                        Else
                            For Each drCard As DataRow In dtResult.Select("CardNo='" & drCUL("CardNo").ToString & "'", "SalesBillID Desc,SalesBillDetailID")
                                ...
                                DB.Open()
                                Dim dt As DataTable
                                dt = DB.GetDataTable("select CardNo from SignCardListNew where CardNo = '" & drCUL("CardNo").ToString & "' and CardType = '实名制卡' and OperationDes='退卡申请'")
                                If dt.Rows.Count > 0 Then
                                    If drCUL("Balance") = drCard("FaceValue") Then
                                        newCard("CheckResult") = "可以撤销"
                                    Else
                                        newCard("CheckResult") = "不可撤销"
                                        newCard("ResultReason") = "卡余额与面值不等！"
                                    End If
                                End If
                                DB.Close()
                                'modify code 069:start-------------------------------------------------------------------------
                    ...
                    If iCards = 0 Then
                        Me.lblInfo2.ForeColor = Color.Red
                        Me.lblInfo2.Text = "查询结束。没有可以撤销激活的购物卡！"
                    Else
                        blStep2OK = True
                        If iCards < dtList1.Compute("Sum(CardQTY)", "") Then
                            Me.lblInfo2.Text = "查询结束。共有 " & Format(iCards, "#,0") & " 张购物卡可以撤销激活，其它卡不可撤销。请按""下一步""选择需要进行激活撤销操作的购物卡对应的销售单号。"
                        Else
                            Me.lblInfo2.Text = "查询结束。该" & IIf(iCards = 1, "张", "批（共 " & Format(iCards, "#,0") & " 张）") & "购物卡可以撤销激活。请按""下一步""选择需要进行激活撤销操作的购物卡对应的销售单号。"
                        End If

                        iCards = dtList1.Compute("Sum(CardQTY)", "")
                        Me.lblInfo1.Text = "共输入了 " & Format(iCards, "#,0") & " 张卡。如果卡号输入完毕，请按""下一步""查看该" & IIf(iCards = 1, "张", "批") & "购物卡的状态及其对应的销售单。"
                    End If
            ...
            Case 2
                ...
                'modify code 047:start-------------------------------------------------------------------------
                If isSignCard Then
                    chkSelectAll_Click(Nothing, Nothing)
                    chkSelectAll.Enabled = False
                    dtList3.Columns(8).ReadOnly = True
                End If
                'modify code 047:end-------------------------------------------------------------------------
            Case 3
                bStep = 4
    ...
    Private Sub btnFinish_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFinish.Click
        Me.Cursor = Cursors.WaitCursor
        Me.lblInfo4.ForeColor = SystemColors.ControlText
        Me.lblInfo4.Text = "正在保存购物卡激活撤销申请记录……"
        Me.Refresh()
        ...
        If Not DB.IsConnected Then
            ...
            Me.lblInfo4.Text = "系统因连接不到数据库而无法保存购物卡激活撤销申请记录。请检查数据库连接。"
            Me.Cursor = Cursors.Default
            Return
        End If
        ...
        If DB.ModifyTable("Select RowID,CardNo,CardState,Balance,SalesBillCode,FaceValue,CardType,StoreName,StoreID,SalesBillID,SalesBillDetailID,Convert(nvarchar(50),Null) As FailureReason Into #tempCard From CULCancelActivation Where 1=2") = -1 Then
            Me.lblInfo4.ForeColor = Color.Red
            Me.lblInfo4.Text = "保存购物卡激活撤销申请记录失败！"
            DB.Close() : Me.Cursor = Cursors.Default : Return
        End If
...
Submit_Again:
        ...
        If dtResult Is Nothing Then
            Me.lblInfo4.ForeColor = Color.Red
            Me.lblInfo4.Text = "保存购物卡激活撤销申请记录时出现数据库内部错误！"
            Me.Cursor = Cursors.Default : Return
        End If
        ...
        If dtResult.Rows(0)("Result").ToString = "Error" Then
            Me.lblInfo4.ForeColor = Color.Red
            Me.lblInfo4.Text = "保存购物卡激活撤销申请记录时出现数据库内部错误！"
            MessageBox.Show("系统无法保存购物卡激活撤销申请记录！下面是数据库的内部错误提示：    " & Chr(13) & Chr(13) & dtResult.Rows(0)("ErrorInfo").ToString & Space(4) & Chr(13) & Chr(13) & "请重试。如果仍有问题，请联系软件开发人员。    ", "保存购物卡激活撤销申请记录时出现数据库内部错误！", MessageBoxButtons.OK, MessageBoxIcon.Stop)
            Me.Cursor = Cursors.Default : Return
        End If
        ...
        ElseIf dtResult.Rows(0)("Result").ToString = "AllOK" Then
            ...
            Next
            sInfo = "已将" & IIf(dtList4.Rows.Count = 1, "该张", "全部") & "购物卡共 " & Format(dtList4.Compute("Sum(FaceValue)", ""), "#,0.00").Replace(".00", "") & " 元加入激活撤销申请。按""开始""执行新的操作，或按""关闭""结束操作。"
        ...
        If dvCard.Count > 0 Then
            Me.lblInfo3.Text = "为确保已申请激活撤销的购物卡不被消费，正对这" & IIf(dvCard.Count = 1, "张", "些") & "购物卡执行冻结操作……"
            Me.Refresh()
            ...
            If iSucesses = 0 Then
                MessageBox.Show("系统在对已申请激活撤销的购物卡执行自动冻结时失败！    " & Chr(13) & Chr(13) & "为确保这" & IIf(dvCard.Count = 1, "张", "些") & "购物卡不被消费，请您手工执行冻结操作。    ", "购物卡自动冻结失败！", MessageBoxButtons.OK, MessageBoxIcon.Stop)
            ElseIf iSucesses < dvCard.Count Then
                MessageBox.Show("系统在对已申请激活撤销的购物卡执行自动冻结时部分失败！    " & Chr(13) & Chr(13) & "为确保这" & IIf(dvCard.Count - iSucesses = 1, "张", "些") & "购物卡不被消费，请您手工执行冻结操作。    ", "购物卡自动冻结失败！", MessageBoxButtons.OK, MessageBoxIcon.Stop)
            End If
            'modify code 047:start-------------------------------------------------------------------------
            If isSignCard AndAlso iSucesses = 1 Then signCardResult = True
            'modify code 047:end-------------------------------------------------------------------------
        End If
