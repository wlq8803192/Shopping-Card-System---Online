Public Class frmFreezeCard
    ...
    Private Sub frmFreezeCard_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'modify code 047:start-------------------------------------------------------------------------
        If isSignCard Then
            txbStartCardNo.Text = signCardNo
            txbStartCardNo.Enabled = False
            txbStartCardNo_Leave(Nothing, Nothing)
            txbEndCardNo.Text = signCardNo
            txbEndCardNo.Enabled = False
            txbEndCardNo_Leave(Nothing, Nothing)
        End If
        'modify code 047:end-------------------------------------------------------------------------
    End Sub
            ...
            Try
                ...
                Do While sStart <= sEndCardNo
                    ...
                    If msg.Code.ToUpper = "IY" OrElse msg.Code.ToUpper = "IZ" Then
                        ...
                        DB.Open()
                        If DB.IsConnected Then
                            strsql = "update SignCardListNew set OperationDes='' where OperationDes='挂失' and CardNo>='" & sStart & "' and CardNo<='" & sEnd & "'"
