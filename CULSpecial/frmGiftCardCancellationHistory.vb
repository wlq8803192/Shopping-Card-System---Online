Public Class frmGiftCardCancellationHistory
    ...
    Private Sub frmGiftCardCancellationHistory_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        frmMain.statusText.Text = "就绪。"
    End Sub

    Private Sub frmGiftCardCancellationHistory_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim DB As New DataBase
        DB.Open()
        If Not DB.IsConnected Then
            frmMain.statusText.Text = "系统因连接不到数据库而无法打开礼品卡""POS机激活撤销""历史记录窗口。请检查数据库连接。"
            Me.Close() : Return
        End If

        Try
            dvList = DB.GetDataTable("Exec GetCULGiftCardCancelActivationHistory " & frmMain.sLoginUserID).DefaultView
        Catch
            frmMain.statusText.Text = "系统因在检索数据时出错而无法打开礼品卡""POS机激活撤销""历史记录窗口。请联系软件开发人员。"
        ...
        If dvList.Count = 0 Then
            MessageBox.Show("还未发现任何礼品卡""POS机激活撤销""历史记录。    ", "没有历史记录。", MessageBoxButtons.OK, MessageBoxIcon.Information)
        ...
        dtState.Rows.Add(3, "撤销失败")
        dtState.Rows.Add(4, "撤销成功")
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        If MessageBox.Show("您确实想删除该卡""" & Me.dgvList("CardNo", Me.dgvList.CurrentRow.Index).Value.ToString & """的激活撤销申请吗？    " & Chr(13) & Chr(13) & "（只能删除由您自己添加的且未被确认的激活撤销申请记录。）    ", "请确认删除激活撤销申请：", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.No Then Return

        Me.Cursor = Cursors.WaitCursor
        frmMain.statusText.Text = "正在删除激活撤销申请记录……"
        ...
        If DB.IsConnected Then
            Dim dtResult As DataTable = DB.GetDataTable("Exec CULGiftCardCancelActivationDelete " & currentRow("OperationID").ToString & "," & frmMain.sSSID)
            DB.Close()
            If dtResult Is Nothing Then
                frmMain.statusText.Text = "删除激活撤销申请记录失败！"
            ElseIf dtResult.Rows(0)("Result").ToString = "AlreadyValidated" Then
                frmMain.statusText.Text = "当前激活撤销申请记录已被他人确认过，您已无法删除它。"
            ElseIf dtResult.Rows(0)("Result").ToString = "Error" Then
                MessageBox.Show("删除激活撤销申请记录时出现错误！下面是数据库的内部错误提示：    " & Chr(13) & Chr(13) & dtResult.Rows(0)("ErrorInfo").ToString & Space(4) & Chr(13) & Chr(13) & "请放弃保存，在关闭窗口后重试。如果仍有问题，请联系软件开发人员。    ", "删除激活撤销申请记录失败！", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                frmMain.statusText.Text = "删除激活撤销申请记录失败！"
            Else
                ...
                If ShoppingCardOperation.CRFCardAutoFreeze(False, dvCULParameter.Table, dtCard) = 0 Then MessageBox.Show("系统在对礼品卡执行自动解冻时失败！    " & Chr(13) & Chr(13) & "为使该卡能被正常使用，请您手工对该卡执行解冻操作。    ", "礼品卡自动解冻失败！", MessageBoxButtons.OK, MessageBoxIcon.Stop)
