Public Class frmCancelMKTActivationHistory

    Private dvList As DataView, dtState As DataTable, blSkipDeal As Boolean = True, blCanBrowseSalesBill = True

    Private Sub frmCancelMKTActivationHistory_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        frmMain.statusText.Text = "就绪。"
    End Sub

    Private Sub frmCancelMKTActivationHistory_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        blCanBrowseSalesBill = (frmMain.dtAllowedRight.Select("RightName='SalesBillBrowse'").Length > 0)

        Dim DB As New DataBase
        DB.Open()
        If Not DB.IsConnected Then
            frmMain.statusText.Text = "系统因连接不到数据库而无法打开活动卡""激活撤销""历史记录窗口。请检查数据库连接。"
            Me.Close() : Return
        End If

        Try
            dvList = DB.GetDataTable("Exec GetCULCancelMKTActivationHistory " & frmMain.sLoginUserID).DefaultView
        Catch
            frmMain.statusText.Text = "系统因在检索数据时出错而无法打开活动卡""激活撤销""历史记录窗口。请联系软件开发人员。"
        ...
        If dvList.Count = 0 Then
            MessageBox.Show("还未发现任何活动卡""激活撤销""历史记录。    ", "没有历史记录。", MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Private Sub cbbStore_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cbbStore.SelectedIndexChanged, cbbDate.SelectedIndexChanged, cbbState.SelectedIndexChanged
        ...
        If dvList.Count = 0 Then
            Me.lblTitle.Text = "活动卡""激活撤销""历史记录："
        Else
            Me.lblTitle.Text = "活动卡""激活撤销""历史记录（共 " & Format(dvList.Count, "#,0") & " 张次 " & Format(dtTemp.Compute("Sum(FaceValue)", ""), "#,0.00").Replace(".00", "") & " 元）："
        End If
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        ElseIf dvList.Table.Select("BatchID=" & Me.dgvList("BatchID", Me.dgvList.CurrentRow.Index).Value.ToString & " And OperationState>1").Length > 0 Then
            MessageBox.Show("由于删除激活撤销申请时将删除同一批次的所有活动卡的申请，    " & Chr(13) & _
                            "但当前卡所在的批次的其它活动卡的处理状态并不一致。    " & Chr(13) & Chr(13) & _
                            "所以不能删除当前卡及同一批次的其它卡的激活撤销申请！    ", _
                            "不能删除激活撤销申请！", MessageBoxButtons.OK, MessageBoxIcon.Stop)
            Me.btnDelete.Enabled = False
        Else
            bAnswer = MessageBox.Show("删除当前卡的激活撤销申请时将同时删除所在批次的其它活动卡的申请。    " & Chr(13) & Chr(13) & _
                                      "所以，您即将删除当前卡所在批次共 " & Format(dvList.Table.Select("BatchID=" & Me.dgvList("BatchID", Me.dgvList.CurrentRow.Index).Value.ToString).Length, "#,0") & " 张（次）活动卡的撤销申请。    " & Chr(13) & Chr(13) & _
                                      "您是否确认删除所在批次的活动卡的激活撤销申请？    ", _
        ...
        If DB.IsConnected Then
            ...
            If blDeleted Then
                ...
                If iSucesses = 0 Then
                    MessageBox.Show("系统在对已取消激活撤销申请的活动卡执行自动解冻时失败！    " & Chr(13) & Chr(13) & "为确保这" & IIf(dtCard.Rows.Count = 1, "张", "些") & "活动卡能被正常使用，请您手工执行解冻操作。    ", "活动卡自动解冻失败！", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                ElseIf iSucesses < dtCard.Rows.Count Then
                    MessageBox.Show("系统在对已取消激活撤销申请的活动卡执行自动解冻时部分失败！    " & Chr(13) & Chr(13) & "为确保这" & IIf(dtCard.Rows.Count - iSucesses = 1, "张", "些") & "活动卡能被正常使用，请您手工执行解冻操作。    ", "活动卡自动解冻失败！", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                End If
