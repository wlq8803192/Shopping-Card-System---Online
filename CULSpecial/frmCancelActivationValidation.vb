Public Class frmCancelActivationValidation
    ...
    Private Sub frmCancelActivationValidation_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            dvList = DB.GetDataTable("Exec GetCULCancelActivationValidating " & frmMain.sLoginUserID).DefaultView
        Catch
                    ...
                    Try
                        ...
                        If codeMsg.Code.ToUpper = "IZ" Then
                            ...
                            If codeMsg.Code.ToUpper = "NZ" Then
                                ...
                                result = DB.ModifyTable("update SignCardListNew set CardType='非实名制卡',OperationDes='' where CardType='实名制卡' and OperationDes='退卡' and CardNo>='" & sStartCardNo & "' and CardNo<='" & sEndCardNo & "'", False)
                                If result > 0 Then
                                    Dim irecClass As New C4ShoppingCardService.IrecClass
                                    irecClass.MerchantNo = sMerchantNo
                                    irecClass.UserID = sMerchantNo
                                    irecClass.CardNoFrom = sStartCardNo
                                    irecClass.CardNoTo = sEndCardNo
                                    Dim codeMsg2 As C4ShoppingCardService.CodeMsg
                                    codeMsg2 = c4Service.irec(irecClass, guIDClass)

                                    If codeMsg2.Code <> "MZ" Then
                                        MessageBox.Show("失败卡号：" & sStartCardNo & "-" & sEndCardNo & vbCrLf & "实名制卡回收申请到CUL执行回收操作时出现故障，请手动回收。    ", "回收时出现故障！", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                                    End If
                                ElseIf result = -1 Then
                                    MessageBox.Show("失败卡号：" & sStartCardNo & "-" & sEndCardNo & vbCrLf & "实名制卡回收操作时出现故障，请手动回收。    ", "回收时出现故障！", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                                End If
                                'modify code 047:end-------------------------------------------------------------------------
                            Else
