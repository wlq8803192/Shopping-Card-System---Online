...
Public Class DataBaseReport
    ...
    Public Sub Open()
        ...
        If SqlConn Is Nothing Then
            If My.Settings.IsInTraining = True Then
                SqlConn = New SqlConnection(frmMain.sTrainingConnection)
            Else
                SqlConn = New SqlConnection(frmMain.sReportConnection)
            End If
    ...
    Public Function GetDataTable(ByVal sSQLString As String) As DataTable
        ...
        Catch ex As Exception
            MessageBox.Show("从数据库中检索不到数据！下面是数据库的内部错误提示：    " & Chr(13) & Chr(13) & ex.Message & Space(4) & Chr(13) & Chr(13) & "请先退出系统后重试。如果仍有问题，请联系软件开发人员。    ", "检索数据库时出错！", MessageBoxButtons.OK, MessageBoxIcon.Stop)
            Return Nothing
        End Try
    End Function
    ...
    Public Function ModifyTable(ByVal sSQLString As String) As Integer
        ...
        Catch ex As Exception
            MessageBox.Show("系统无法更新数据到数据库！下面是数据库的内部错误提示：    " & Chr(13) & Chr(13) & ex.Message & Space(4) & Chr(13) & Chr(13) & "请先退出系统后重试。如果仍有问题，请联系软件开发人员。    ", "更新数据库时出错！", MessageBoxButtons.OK, MessageBoxIcon.Stop)
            Return -1
        End Try
    End Function
