Public Class frmEmployeeCardActivation
    ...
    Private sProcedureName As String = "GetActivationList_EMP", sSummaryInfo As String = "", sRowFilter As String = "", sTimerType As String = "DoubleClick", bClicks As Byte = 0, sSalesDateError As String = "", sSelectedSalesBillDetailID As String = ""
    ...
    Private Sub frmEmployeeCardActivation_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
    ...
    Private Sub frmEmployeeCardActivation_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Me.Dispose()
        frmMain.statusText.Text = "就绪。"
    End Sub

    Private Sub frmEmployeeCardActivation_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If frmMain.dtLoginStructure.Select("AreaType='S'").Length = 0 Then
            MessageBox.Show("没有门店，不能打开""员工卡激活""窗口！    " & Chr(13) & Chr(13) & "请找系统管理员建立门店。    ", "未发现门店！", MessageBoxButtons.OK, MessageBoxIcon.Stop)
            Me.Close() : Return
        End If

        blCanActivate = (frmMain.dtAllowedRight.Select("RightName='EMPCardActivate'").Length > 0)
        ...
        If dtStore.Rows.Count = 1 Then
            ...
            Me.pnlSalesDate.Left = 11
        End If

        Dim DB As New DataBase
        DB.Open()
        If Not DB.IsConnected Then
            frmMain.statusText.Text = "系统因连接不到数据库而无法打开""员工卡激活""窗口。请检查数据库连接。"
            Me.Close() : Return
        End If
        ...
        Dim sNeedActivateMinDate As String = ""
        Try
            ...
            If blCanActivate Then
                Try
                    sNeedActivateMinDate = Format(dvList.Table.Compute("Min(SalesDate)", "CardStates Like '1%'"), "yyyy\/MM\/dd")
                Catch
                End Try
            End If
        ...
        If sNeedActivateMinDate <> "" Then
            blSkipDeal = True
            Me.cbbStore.SelectedValue = dvList.Table.Select("SalesDate='" & sNeedActivateMinDate & "' And CardStates Like '1%'")(0)("StoreID")
            Me.cbbSalesDate.SelectedIndex = -1
            Me.cbbSalesDate.Text = sNeedActivateMinDate
    ...
    Private Sub cbbSalesDate_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cbbSalesDate.SelectedIndexChanged
        ...
        If Me.cbbStore.SelectedValue <> -1 AndAlso IsDate(Me.cbbSalesDate.Text) Then
            ...
            If blCanActivate Then
                ...
                If dvList.Table.Compute("Count(SalesBillDetailID)", "StoreID=" & Me.cbbStore.SelectedValue.ToString & " And SalesDate='" & Me.cbbSalesDate.Text & "' And CardStates Like '1%'") > 0 Then
                    sRowFilter = "(SalesDate='" & Me.cbbSalesDate.Text & "')"
                    Me.btnActivate.Enabled = True
                End If
            End If
        Else
    ...
    Private Sub btnRefresh_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnRefresh.Click
        ...
        Me.btnActivate.Enabled = False
        If Me.chbDisplaySelected.Visible Then
            sRowFilter = "(SalesDate='" & Me.cbbSalesDate.Text & "')"
            Me.btnActivate.Enabled = True
        End If
    ...
    Private Sub chbDisplaySelected_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chbDisplaySelected.CheckedChanged
        ...
        With Me.dgvList
            ...
            .Columns("PaymentValidationState").Visible = .Columns("MediaNo").Visible
    ...
    Private Sub btnActivate_EnabledChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnActivate.EnabledChanged
        If Not Me.btnActivate.Enabled Then Return
        ...
        For Each cardRow As DataGridViewRow In Me.dgvList.Rows
            blSkipDeal = True
            Me.dgvList.CurrentCell = cardRow.Cells("SalesBillCode")
            cardRow.Selected = False
            blSkipDeal = False
            cardRow.Selected = True

            If Me.chbSelectAllCards.Visible Then
                blSkipDeal = True
    ...
    Private Sub SetRowFilter()
        ...
        If Me.cbbPaymentTerm.SelectedIndex > 0 Then sRowFilter = IIf(sRowFilter = "", "", sRowFilter & " And ") & "(PaymentTermName='" & Me.cbbPaymentTerm.Text & "')"
    ...
    Private Function SaveChanges() As Boolean
        ...
        Dim sPrompt As String = "当天（" & Me.cbbSalesDate.Text & "）要激活的员工销售单汇总信息如下：    " & Chr(13) & Chr(13)
        ...
        If MessageBox.Show(sPrompt, "请确认激活：", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Cancel Then
            'For Each cardRow As DataGridViewRow In Me.dgvList.Rows
            '    blSkipDeal = True
            '    Me.dgvList.CurrentCell = cardRow.Cells("SalesBillCode")
            '    cardRow.Selected = False
