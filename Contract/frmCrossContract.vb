...
Public Class frmCrossContract
    ...
    Private Sub frmCrossContract_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub frmCrossContract_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            Dim LoginAreaID As String = frmMain.sLoginAreaID
            ...
            If sContractID = "-1" Then
                drContract = DB.GetDataTable("Exec CrossContractSingle -1").Rows.Add
                dtToday = Today()
                If sCustomerID = "" Then
                    drContract("StartDate") = dtToday
                    If frmMain.sLoginAreaType = "S" Then drContract("SignPlace") = sCityName
                Else
                    Dim dtCustomer As DataTable = DB.GetDataTable("Exec CrossCustomerSingle " & sCustomerID).Copy
                ...
                dtContractDetails = DB.GetDataTable("Select * From CrossContractDetails Where 1=2").Copy
                dtContractDetails.Rows.Add(-1, 1, 0, DBNull.Value, 0)

                dvNewestCityRule = DB.GetDataTable("Select * From CrossSellingCityRule Where CityID=" & sCityID & " And RuleState=2").DefaultView
                If dvNewestCityRule.Count = 0 Then
                    dvNewestRuleDetails = DB.GetDataTable("Select * From CrossSellingCityRuleDetails Where RuleID=-1").DefaultView
                Else
                    dvNewestRuleDetails = DB.GetDataTable("Select * From CrossSellingCityRuleDetails Where RuleID=" & dvNewestCityRule(0)("RuleID").ToString).DefaultView
                End If
...
Open_Contract:
                Me.Text = "合同 Contract: "
                Dim dtContract As DataTable = DB.GetDataTable("Exec CrossContractSingle " & sContractID)
                If dtContract.Rows.Count = 0 Then
                    ...
                    If frmCrossContractManagement.IsHandleCreated Then
                        Try
                            frmCrossContractManagement.dvContract.Table.Rows.Find(sContractID).Delete()
                        Catch
                        End Try
                    End If
                    sContractID = ""
                ...
                If dtContractDetails Is Nothing Then dtContractDetails = DB.GetDataTable("Select * From CrossContractDetails Where ContractID=" & sContractID).Copy
                ...
                Next
                If sRuleIDs = "" Then
                    dvHistoryCityRule = DB.GetDataTable("Select * From CrossSellingCityRule Where RuleID=-1").DefaultView
                Else
                    sRuleIDs = sRuleIDs.Substring(0, sRuleIDs.Length - 1)
                    dvHistoryCityRule = DB.GetDataTable("Select * From CrossSellingCityRule Where RuleID In (" & sRuleIDs & ")").DefaultView
                End If
                If dvHistoryCityRule.Count = 0 Then
                    dvHistoryRuleDetails = DB.GetDataTable("Select * From CrossSellingCityRuleDetails Where RuleID=-1").DefaultView
                Else
                    If sRuleIDs = "" Then
                        ...
                        sRuleIDs = sRuleIDs.Substring(0, sRuleIDs.Length - 1)
                    End If
                    dvHistoryRuleDetails = DB.GetDataTable("Select * From CrossSellingCityRuleDetails Where RuleID In (" & sRuleIDs & ")").DefaultView
                End If
                ...
                dvNewestCityRule = DB.GetDataTable("Select * From CrossSellingCityRule Where CityID In (" & sCityIDs & ") And RuleState=2").DefaultView
                If dvNewestCityRule.Count = 0 Then
                    dvNewestRuleDetails = DB.GetDataTable("Select * From CrossSellingCityRuleDetails Where RuleID=-1").DefaultView
                Else
                    If sRuleIDs = "" Then
                        ...
                        sRuleIDs = sRuleIDs.Substring(0, sRuleIDs.Length - 1)
                    End If
                    dvNewestRuleDetails = DB.GetDataTable("Select * From CrossSellingCityRuleDetails Where RuleID In (" & sRuleIDs & ")").DefaultView
                End If
                Me.CheckValidation()

                blCanModify = ("|0|1|3|".IndexOf("|" & drContract("ContractState").ToString & "|") > -1 AndAlso frmMain.dtAllowedRight.Select("RightName='CrossContractModify'").Length > 0)
                If blCanModify Then
                    For Each joinedCustomer As DataRow In dtContractCustomer.Rows
                        sCustomerID = joinedCustomer("CustomerID").ToString
                        drSelectedCustomer = DB.GetDataTable("Exec CrossCustomerSingle " & sCustomerID).Rows(0)
                        If frmCustomerManagement.IsHandleCreated Then frmCustomerManagement.UpdateCustomer(drSelectedCustomer)
                        If drSelectedCustomer("LastValidContractID").ToString = "" Then
                            joinedCustomer("LastValidContractID") = -1
                        Else
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        If frmMain.dtAllowedRight.Select("RightName='CrossContractPrint'").Length = 0 Then
    ...
    Private Sub rdbFirstFailure_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rdbFirstFailure.CheckedChanged
        ...
        If Me.rdbFirstFailure.Checked Then
            ...
            blCanModify = (frmMain.dtAllowedRight.Select("RightName='CrossContractModify'").Length > 0)
            Me.ResetControls()
            ...
            Else
                Select Case dtDropdownCustomer.Rows.Count
                    ...
                    Case 1
                        ...
                        End If
                        Dim dtCustomer As DataTable = DB.GetDataTable("Exec CrossCustomerSingle " & sCustomerID)
    ...
    Private Sub cbbCustomerName_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbbCustomerName.SelectedIndexChanged
        ...
        DB.Open()
        If DB.IsConnected Then
            dtCustomer = DB.GetDataTable("Exec CrossCustomerSingle " & sCustomerID)
            If dtCustomer Is Nothing Then
                sCustomerID = ""
                sCustomerPrompt = "提示：您可以输入客户编号、客户名称中的部分文字或客户名称拼音码来确定客户。"
                frmMain.statusText.Text = sCustomerPrompt
    ...
    Private Sub btnJoin_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnJoin.Click
        ...
        If frmMain.dtAllowedRight.Select("RightName='CityConfigBrowse'").Length > 0 AndAlso sCityID <> sNewCityID AndAlso dvNewestCityRule.Table.Select("CityID=" & sNewCityID).Length = 0 Then
            ...
            If DB.IsConnected Then
                Try
                    dvNewestCityRule.Table.Merge(DB.GetDataTable("Select * From CrossSellingCityRule Where CityID=" & sNewCityID & " And RuleState=2"))
                    dvNewestCityRule.RowFilter = "CityID=" & sNewCityID
                    If dvNewestCityRule.Count > 0 Then
                        dvNewestRuleDetails.Table.Merge(DB.GetDataTable("Select * From CrossSellingCityRuleDetails Where RuleID=" & dvNewestCityRule(0)("RuleID").ToString))
                    End If
                Catch
    ...
    Private Sub txbMaxFaceValue_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txbMaxFaceValue.Validating
        ...
        If CDec(Me.txbMaxFaceValue.Text) > 1000 Then
            ...
            If CDec(Me.txbMaxFaceValue.Text) <> 5000 Then
    ...
    Private Sub cbbInvoiceItem_DropDown(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbbInvoiceItem.DropDown
        ...
        Else
            ...
            Loop Until theParent.Name = "frmCrossContract"
    ...
    Private Sub FillContract()
        ...
        Else
            ...
            If frmCrossContractManagement.IsHandleCreated Then
                Dim contractRow As DataRow = frmCrossContractManagement.dvContract.Table.Rows.Find(sContractID)
    ...
    Private Sub CheckValidation()
        ...
        Else
            ...
            Next
        End If
        blCanStop = (blCanValidate AndAlso frmMain.dtAllowedRight.Select("RightName='CrossContractStop'").Length > 0) '操作者必须同时具有审核和停用的权限才能停用合同
    ...
    Private Function SaveChanges() As Boolean
        ...
        If sContractID = "-1" Then '新建合同
            ...
            If DB.ModifyTable("Select RowID,CustomerID,InvoiceTitleCustomerID,InvoiceItem Into #tempCustomer From CrossContractCustomer Where 1=2") = -1 Then
            ...
            If DB.ModifyTable("Select LevelID,StartSalesAMT,EndSalesAMT,RebateRate Into #tempContractDetails From CrossContractDetails Where 1=2") = -1 Then
            ...
            sSQL = "Exec CrossContractInsertion @ACompanyChineseName='" & drContract("ACompanyChineseName").ToString.Replace("'", "''") & "',"
            ...
            ElseIf dtResult.Rows(0)("Result").ToString = "OK" Then
                ...
                If frmCrossContractManagement.IsHandleCreated Then
                    Dim newContract As DataRowView = frmCrossContractManagement.dvContract.AddNew
                    ...
                    If (frmCrossContractManagement.cbbCity.SelectedIndex <> 0 AndAlso frmCrossContractManagement.cbbCity.Text <> newContract("CityNameBelongs").ToString) OrElse (frmCrossContractManagement.cbbState.SelectedIndex <> 0 AndAlso frmCrossContractManagement.cbbState.Text <> newContract("ContractStateDescription").ToString) Then
                        frmCrossContractManagement.cbbCity.SelectedIndex = 0
                        frmCrossContractManagement.cbbState.SelectedIndex = 0
                    End If
                    For Each dr As DataGridViewRow In frmCrossContractManagement.dgvList.Rows
                    ...
                    Next
                    newContract = Nothing
                    With frmCrossContractManagement.dgvList.Columns("CustomerName")
                        .MinimumWidth = 2
                        .AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells
                        .MinimumWidth = .Width
                        .AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill
                ...
                Me.txbContratState.Select() : Me.txbContratState.SelectAll()
                Me.btnSave.Enabled = False
                Me.btnPrint.Enabled = (frmMain.dtAllowedRight.Select("RightName='CrossContractPrint'").Length > 0)
        ...
        ElseIf Me.pnlFirstValidate.Visible = False AndAlso Me.pnlHOValidate.Visible = False AndAlso Me.pnlValidate.Visible = False AndAlso Me.pnlStop.Visible = False Then '修改合同
            If dtContractCustomer.GetChanges() IsNot Nothing Then
                ...
                If DB.ModifyTable("Select ContractID,CustomerID,InvoiceTitleCustomerID,InvoiceItem Into #tempCustomer From CrossContractCustomer Where 1=2") = -1 Then
                    dtTempCustomer.Dispose() : dtTempCustomer = Nothing : DB.Close()
                    frmMain.statusText.Text = "保存合同失败！"
                    Me.Cursor = Cursors.Default
                    Return False
            ...
            sSQL = "Exec CrossContractUpdate @ContractID=" & sContractID & ","
            ...
            ElseIf dtResult.Rows(0)("Result").ToString = "AlreadyChanged" Then
                ...
                Me.Refresh()
                Try
                    Dim updatedContract As DataTable = DB.GetDataTable("Exec CrossContractSingle " & sContractID).Copy
                    Dim updatedCustomer As DataTable = DB.GetDataTable("Select CustomerID,InvoiceTitleCustomerID,InvoiceItem From CrossContractCustomer Where ContractID=" & sContractID).Copy, joinedCustomer As DataRow
                    ...
                    Next
                    Dim updatedDetails As DataTable = DB.GetDataTable("Select * From CrossContractDetails Where ContractID=" & sContractID).Copy
            ...
            ElseIf dtResult.Rows(0)("Result").ToString = "OK" Then
                ...
                Me.grbPartyA.Select()
                Me.txbContratState.Select() : Me.txbContratState.SelectAll()
                Me.btnSave.Enabled = False : Me.btnPrint.Enabled = (frmMain.dtAllowedRight.Select("RightName='CrossContractPrint'").Length > 0)
        ...
        ElseIf Me.pnlFirstValidate.Visible OrElse Me.pnlHOValidate.Visible OrElse Me.pnlValidate.Visible Then '审核合同
            ...
            If blMultiCompanies = False OrElse drContract("ContractState") < 3 Then '审核或初审合同时
                If DB.ModifyTable("Select ContractID,CustomerID,Convert(smallint,Null) As CityID,CityRuleID,Convert(int, Null) As NewCityRuleID Into #tempCustomer From CrossContractCustomer Where 1=2") = -1 Then
...
Execute_Again:
            ...
            sSQL = "Exec CrossContractValidate @ContractID=" & sContractID & ","
            If drContract("ContractState", DataRowVersion.Original) <> 0 Then sSQL = sSQL & "@OldContractState=" & drContract("ContractState", DataRowVersion.Original).ToString & ","
            If drContract("ModifiedTime").ToString = "" Then
                sSQL = sSQL & "@OldModifiedTime='" & Format(drContract("CreatedTime"), "yyyy\/MM\/dd HH:mm:ss") & "',"
            Else
            ...
            ElseIf dtResult.Rows(0)("Result").ToString = "CityRuleChanged" Then
                ...
                Try
                    ...
                    Next
                    sCityIDs = sCityIDs.Substring(0, sCityIDs.Length - 1)
                    dtNewestCityRule = DB.GetDataTable("Select * From CrossSellingCityRule Where CityID In (" & sCityID & ") And RuleState=2").Copy
                    If dtNewestCityRule.Rows.Count = 0 Then
                        dtNewestRuleDetails = DB.GetDataTable("Select * From CrossSellingCityRuleDetails Where RuleID=-1").Copy
                    Else
                        If sRuleIDs = "" Then
                            ...
                            sRuleIDs = sRuleIDs.Substring(0, sRuleIDs.Length - 1)
                        End If
                        dtNewestRuleDetails = DB.GetDataTable("Select * From CrossSellingCityRuleDetails Where RuleID In (" & sRuleIDs & ")").Copy
                    End If
            ...
            ElseIf dtResult.Rows(0)("Result").ToString = "OK" Then
                ...
                If drContract("ContractState") = 4 AndAlso drContract("StartDate") <= dtToday Then
                    ...
                    Me.txbContratState.Text = "已审核且已生效 Validated and effective"
                    blCanValidate = False
                    blCanStop = (frmMain.dtAllowedRight.Select("RightName='CrossContractStop'").Length > 0)
                    Me.pnlStop.Visible = blCanStop
        ...
        Else '停用合同
            ...
            sSQL = "Exec CrossContractStop @ContractID=" & sContractID & ",@StateReason='" & drContract("StateReason").ToString.Replace("'", "''") & "',@OperatorName='" & frmMain.sLoginUserRealName.Replace("'", "''") & "',@SSID=" & frmMain.sSSID
            dtResult = DB.GetDataTable(sSQL)
            ...
            Else
                ...
                If frmCrossContractManagement.IsHandleCreated Then
                    Try
                        ...
                        contractRow.EndEdit() : contractRow.AcceptChanges()
                        contractRow = Nothing
                        frmCrossContractManagement.dgvList.Columns("StoppedDate").Visible = True
                    Catch
                    End Try
                End If
