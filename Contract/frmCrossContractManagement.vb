Public Class frmCrossContractManagement
    'modify code 067:
    'date;2016/10/17
    'auther:Qipeng
    'remark:新添加通卖合同管理
    ...
    Private Sub frmCrossContractManagement_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        If frmMain.statusText.Text.IndexOf("无法") = -1 Then frmMain.statusText.Text = "就绪。"
    End Sub

    Private Sub frmCrossContractManagement_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim DB As New DataBase()
        DB.Open()
        If Not DB.IsConnected Then
            frmMain.statusText.Text = "系统因连接不到数据库而无法打开""通卖合同管理""窗口。请检查数据库连接。"
            Me.Close() : Return
        End If
        ...
        Try
            ...
            If frmMain.sLoginAreaType = "S" Then
                If frmMain.sLoginRoleID = "14" Then
                    dvContract = DB.GetDataTable("Select * From CrossContractView Where CreatorID=" & frmMain.sLoginUserID).DefaultView
                ElseIf sControllableBusinessTypes = "All" Then
                    dvContract = DB.GetDataTable("Select * From CrossContractView Where StoreID=" & frmMain.sLoginAreaID).DefaultView
                Else
                    dvContract = DB.GetDataTable("Select * From CrossContractView Where StoreID=" & frmMain.sLoginAreaID & " And '" & sControllableBusinessTypes & "' Like '%|'+Convert(varchar(10),BusinessTypeID)+'|%'").DefaultView
                End If
            ElseIf frmMain.sLoginAreaType = "C" Then
                If sControllableBusinessTypes = "All" Then
                    dvContract = DB.GetDataTable("Select * From CrossContractView Where CityID=" & frmMain.sLoginAreaID).DefaultView
                Else
                    dvContract = DB.GetDataTable("Select * From CrossContractView Where CityID=" & frmMain.sLoginAreaID & " And '" & sControllableBusinessTypes & "' Like '%|'+Convert(varchar(10),BusinessTypeID)+'|%'").DefaultView
                End If
            Else
                If sControllableBusinessTypes = "All" Then
                    dvContract = DB.GetDataTable("Select C.* From CrossContractView AS C Join AreaScope(" & frmMain.sLoginUserID & ") AS S ON C.CityID=S.AreaID").DefaultView
                Else
                    dvContract = DB.GetDataTable("Select C.* From CrossContractView AS C Join AreaScope(" & frmMain.sLoginUserID & ") AS S ON C.CityID=S.AreaID And '" & sControllableBusinessTypes & "' Like '%|'+Convert(varchar(10),C.BusinessTypeID)+'|%'").DefaultView
                End If
            End If
        ...
        Catch
            frmMain.statusText.Text = "系统因在检索数据时出错而无法打开""通卖合同管理""窗口。请联系软件开发人员。"
        ...
        Me.btnAdd.Enabled = (frmMain.dtAllowedRight.Select("RightName='CrossContractCreate'").Length > 0)
        blCanDelete = (frmMain.dtAllowedRight.Select("RightName='CrossContractDelete'").Length > 0)
        ...
        ElseIf frmMain.dtAllowedRight.Select("RightName='CrossContractValidate'").Length > 0 Then
    ...
    Private Sub btnOpen_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOpen.Click
        ...
        frmCrossContract.sContractID = openingRow("ContractID").ToString
    ...
    Private Sub btnAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAdd.Click
        If frmMain.dtAllowedRight.Select("RightName='CrossContractCreate'").Length = 0 Then
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        If frmMain.dtAllowedRight.Select("RightName='CrossContractDelete'").Length = 0 Then
        ...
        If DB.IsConnected Then
            Dim dtResult As DataTable = DB.GetDataTable("Exec CrossContractDeletion " & deletingRow("ContractID").ToString & "," & frmMain.sSSID)
    ...
    Private Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        frmSearchCrossContract.ShowDialog()
        frmSearchCrossContract.Dispose()
    End Sub
End Class