Public Class frmConfigInvoice
    ...
    Private Sub frmConfigInvoice_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        If frmSelling.dtInvoiceLayout.Rows(0)("IsLocked") Then
        ...
        ElseIf "|21|272|".IndexOf("|" & frmMain.dtLoginStructure.Rows.Find(frmSelling.drSalesBill("StoreID").ToString)("ParentAreaID").ToString & "|") > -1 Then '上海
            Me.lblTitle.Text = "上海市门店只需指定发票抬头的位置（将应用于全店的所有机台）："
        ElseIf frmMain.dtLoginStructure.Rows.Find(frmSelling.drSalesBill("StoreID").ToString)("ParentAreaID").ToString = "24" Then '重庆
            Me.lblTitle.Text = "重庆市门店只需指定发票抬头的位置（将应用于全店的所有机台）："
        ElseIf frmMain.dtLoginStructure.Rows.Find(frmSelling.drSalesBill("StoreID").ToString)("ParentAreaID").ToString = "27" Then '武汉
            Me.lblTitle.Text = "武汉市门店只需指定发票抬头的位置（将应用于全店的所有机台）："
        End If

        With Me.dgvInvoice
            .DataSource = frmSelling.dtInvoiceLayout
            ...
            End If
        End With
        For bColumn As Byte = 0 To frmSelling.dtInvoiceLayout.Columns.Count - 1
            Me.dgvInvoice.Columns(bColumn).Name = frmSelling.dtInvoiceLayout.Columns(bColumn).ColumnName
        Next
        ...
        Me.btnOK.Enabled = (Me.cbbPrinter.Text.IndexOf("（不可用）") = -1 AndAlso (Me.cbbPrinter.Text <> My.Settings.InvoicePrinter OrElse frmSelling.dtInvoiceLayout.GetChanges() IsNot Nothing))
    ...
    Private Sub dgvInvoice_CellValueChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvInvoice.CellValueChanged
        ...
        If Me.dgvInvoice.Columns(e.ColumnIndex).Name = "FontSize" AndAlso Me.dgvInvoice(e.ColumnIndex, e.RowIndex).Value < 8 Then
            ...
            blSkipDeal = False
        End If
        Dim currentRow As DataRow = frmSelling.dtInvoiceLayout.Rows(e.RowIndex)
    ...
    Private Sub btnPrintTest_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrintTest.Click
        ...
        Try
            ...
            If "|21|24|".IndexOf("|" & frmMain.dtLoginStructure.Rows.Find(frmSelling.drSalesBill("StoreID").ToString)("ParentAreaID").ToString & "|") > -1 Then '上海/重庆
                AddHandler testPrint.PrintPage, AddressOf testPrintSH_PrintPage
            ElseIf "|27|272|".IndexOf("|" & frmMain.dtLoginStructure.Rows.Find(frmSelling.drSalesBill("StoreID").ToString)("ParentAreaID").ToString & "|") > -1 Then '武汉/测试城市
    ...
    Private Sub btnOK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOK.Click
        If frmSelling.dtInvoiceLayout.GetChanges() IsNot Nothing AndAlso MessageBox.Show("注意：发票打印版面一旦保存，便不再可修改！    " & Chr(13) & Chr(13) & "请确保所有的设置均已通过测试。是否现在就保存？    ", "请确认保存发票版面设置：", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.No Then Return
        ...
        If My.Settings.InvoicePrinter <> Me.cbbPrinter.Text Then
            ...
            If sPortName.ToUpper.IndexOf("IP_") = 0 Then
                frmSelling.sInvoicePrinterDevice = sPortName
            ElseIf Net.IPAddress.TryParse(sPortName, ipAddress) Then
                frmSelling.sInvoicePrinterDevice = "IP_" & sPortName 'Win7环境下的网络打印机本地化为以IP地址为名称的端口名
            ElseIf My.Settings.InvoicePrinter.IndexOf("\\") = 0 Then
                frmSelling.sInvoicePrinterDevice = My.Settings.InvoicePrinter
            Else '将本地打印机表达成网络打印机的形式
                frmSelling.sInvoicePrinterDevice = "\\" & sSystemName & "\" & My.Settings.InvoicePrinter
            End If
            Try
                Dim drInvoiceCodeNo As DataRow = DB.GetDataTable("Select LastInvoiceCode,LastInvoiceNo From InvoiceCodeNo Where StoreID=" & frmMain.sLoginAreaID & " And PrinterDevice='" & frmSelling.sInvoicePrinterDevice.Replace("'", "''") & "'").Rows(0)
                frmSelling.sInvoiceCode = drInvoiceCodeNo("LastInvoiceCode").ToString
                frmSelling.sInvoiceNo = (CLng(drInvoiceCodeNo("LastInvoiceNo").ToString) + 1).ToString
        ...
        If frmSelling.dtInvoiceLayout.GetChanges() IsNot Nothing Then
            For Each drItem As DataRow In frmSelling.dtInvoiceLayout.Rows
                drItem("IsLocked") = 1
            Next

            If DB.ModifyTable("Delete From InvoiceLayout Where StoreID=" & frmSelling.drSalesBill("StoreID").ToString) = -1 OrElse _
               DB.BulkCopyTable("InvoiceLayout", frmSelling.dtInvoiceLayout) = -1 Then
                frmMain.statusText.Text = "保存发票设置失败！"
            Else
                DB.ModifyTable("Exec OperationLogInsert " & frmMain.sSSID & ",'Config Invoice Layout','Store'," & frmSelling.drSalesBill("StoreID").ToString)
                frmSelling.dtInvoiceLayout.AcceptChanges()
                Me.lblTitle.Text = "各打印项目的位置及其可见性（不可修改）："
                Me.dgvInvoice.ReadOnly = True
    ...
    Private Sub testPrint_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs)
        ...
        For Each drItem As DataRow In frmSelling.dtInvoiceLayout.Select("IsVisible=1")
            Select Case drItem("ItemName").ToString
                ...
                Case "计量数量"
                    sText = "1"
                Case "计量单价"
                    sText = Format(frmSelling.drSalesBill("PayableAMT"), "#,0.00")
                Case "发票金额"
                    sText = Format(frmSelling.drSalesBill("PayableAMT"), "#,0.00")
                Case "大写总额"
                    sText = CapitalDigits.CapitalRMB(frmSelling.drSalesBill("PayableAMT"))
                Case "付款方式"
                    sText = ""
                    For Each drPayment As DataGridViewRow In frmSelling.dgvPayment.Rows
                        sText = sText & drPayment.Cells("PaymentTerm").FormattedValue.ToString & "/"
                    Next
                ...
                Case "家乐福小票号（含标题）"
                    sText = "家乐福小票号：" & frmSelling.lblSalesBillCode.Text.Replace(" ", "")
                Case "购买统计信息（含标题）"
                    sText = "总购买金额：" & Format(frmSelling.drSalesBill("ChargedAMT"), "#,0.00") & "  折扣金额：" & Format(frmSelling.drSalesBill("RebateSalesAMT"), "#,0.00") & "  实付金额：" & Format(frmSelling.drSalesBill("PayableAMT"), "#,0.00")
            End Select
    ...
    Private Sub testPrintSH_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs)
        ...
        e.Graphics.DrawString("家乐福小票号：" & frmSelling.lblSalesBillCode.Text.Replace(" ", ""), drawFont, drawBrush, x, y, drawFormat)
        x -= 167 : y += 8 '付款单位名称
        ...
        x += 25 '单价
        e.Graphics.DrawString(Format(frmSelling.drSalesBill("PayableAMT"), "#,0.00"), drawFont, drawBrush, x, y, drawFormat)
        x += 25 '总价
        e.Graphics.DrawString(Format(frmSelling.drSalesBill("PayableAMT"), "#,0.00"), drawFont, drawBrush, x, y, drawFormat)
        drawFormat.Alignment = StringAlignment.Near
        y += 5 : x -= 18 '折扣
        ...
        x += 18 '折扣金额
        drawFormat.Alignment = StringAlignment.Center
        e.Graphics.DrawString(Format(frmSelling.drSalesBill("RebateSalesAMT"), "#,0.00"), drawFont, drawBrush, x, y, drawFormat)
        y += 5 : x -= 18 '实付金额
        drawFormat.Alignment = StringAlignment.Far
        ...
        drawFormat.Alignment = StringAlignment.Near
        x -= 149 : y += 40 '大写金额
        e.Graphics.DrawString("合计金额大写（人民币）：" & CapitalDigits.CapitalRMB(frmSelling.drSalesBill("PayableAMT")), drawFont, drawBrush, x, y, drawFormat)
        drawFormat.Alignment = StringAlignment.Far
        x += 160 '小写金额
        e.Graphics.DrawString("合计金额小写：" & Format(frmSelling.drSalesBill("PayableAMT"), "#,0.00"), drawFont, drawBrush, x, y, drawFormat)
        drawFormat.Alignment = StringAlignment.Near
        x -= 160 : y += 16 '开票人
