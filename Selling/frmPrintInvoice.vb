Public Class frmPrintInvoice
    ...
    Public dtInvoiceItem As DataTable, blIsDeduction As Boolean = False, dmTotalChargedAMT As Decimal = 0, dmTotalRebateAMT As Decimal = 0, sCarrefourName As String, sReceiverTaxNo As String

    Private Sub frmPrintInvoice_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        If frmMain.statusText.Text.IndexOf("无法") = -1 Then frmMain.statusText.Text = "就绪。"
    End Sub

    Private Sub frmPrintInvoice_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        If dmReprintableAMT > 0 Then
            Dim bAnswer As DialogResult = Windows.Forms.DialogResult.Cancel
            If frmSelling.dtInvoice.Select("Remarks Like '剩余金额%'").Length > 0 Then
            ...
            If bAnswer = Windows.Forms.DialogResult.OK Then
                frmSelling.dtInvoice.RejectChanges()
            Else
    ...
    Private Sub frmPrintInvoice_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            dmReprintableAMT = frmSelling.dtInvoice.Compute("Sum(ReprintableAMT)", "")
        Catch
            dmReprintableAMT = 0
        End Try

        With Me.dgvInvoice
            .DataSource = frmSelling.dtInvoice
            With .Columns(0)
                .HeaderText = "行号"
                .Width = 40
            ...
            With .Columns(10)
                ...
                .SortMode = DataGridViewColumnSortMode.NotSortable
                .Resizable = DataGridViewTriState.False
                .Visible = (frmSelling.dtInvoice.Select("Isnull(SourceInvoice,'')<>''").Length > 0)
            End With
            With .Columns(11)
                .HeaderText = "状态"
                .AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells
            ...
            With .Columns(12)
                ...
                .SortMode = DataGridViewColumnSortMode.NotSortable
                .Resizable = DataGridViewTriState.False
                .Visible = (frmSelling.dtInvoice.Select("Isnull(CancelledReason,'')<>''").Length > 0)
            End With
            With .Columns(13)
                .HeaderText = "说明"
                .AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells
                .SortMode = DataGridViewColumnSortMode.NotSortable
                .Resizable = DataGridViewTriState.False
                .Visible = (frmSelling.dtInvoice.Select("Isnull(Remarks,'')<>''").Length > 0)
            End With
            ...
            With .Columns(14)
                ...
                .SortMode = DataGridViewColumnSortMode.NotSortable
                .Resizable = DataGridViewTriState.False
                .Visible = (frmMain.dtAllowedRight.Select("RightName='SalesBillInvoiceReprint'").Length > 0)
            End With
            .Columns(15).Visible = False
            .Columns(16).Visible = False
            .Columns(17).Visible = False
        End With
        For bColumn As Byte = 0 To frmSelling.dtInvoice.Columns.Count - 1
            Me.dgvInvoice.Columns(bColumn).Name = frmSelling.dtInvoice.Columns(bColumn).ColumnName
        Next

        If frmMain.dtAllowedRight.Select("RightName Like 'SalesBillInvoice%'").Length > 0 Then
            With Me.cbbInvoiceItem1
                For Each drItem As DataRow In dtInvoiceItem.Select("", "ItemID")
            ...
            With Me.cbbInvoiceItem2
                .Items.Add("（没有品项）")
                If dtInvoiceItem.Rows.Count = 1 Then
                    .SelectedIndex = 0
                Else
                    For Each drItem As DataRow In dtInvoiceItem.Select("Content<>'" & Me.cbbInvoiceItem1.Text.Replace("'", "''") & "'", "ItemID")
                        .Items.Add(drItem("Content").ToString)
                    Next
        ...
        If dmAvailableAMT + dmReprintableAMT = 0 OrElse frmMain.dtAllowedRight.Select("RightName Like 'SalesBillInvoice%'").Length = 0 Then
        ...
        Else
            ...
            If Not frmSelling.drInvoicePrintable("CanPrintInNextMonth") AndAlso Today() > CDate(Format(DateAdd(DateInterval.Month, 1, frmSelling.drSalesBill("CreatedTime")), "yyyy\/MM\/dd").Substring(0, 8) & "01") Then
            ...
            ElseIf DateDiff(DateInterval.Day, frmSelling.drSalesBill("CreatedTime"), Today()) > 31 Then
                For Each ctrl As Control In Me.grbNewInvoice.Controls
                    ctrl.Enabled = False
                Next
                For Each ctrl As Control In Me.grbNewInvoiceExtra.Controls
                ...
                Me.lblError.Visible = True
            ElseIf dmAvailableAMT + dmReprintableAMT < frmSelling.drInvoicePrintable("MinInvoiceAMT") Then
                For Each ctrl As Control In Me.grbNewInvoice.Controls
                    ctrl.Enabled = False
                Next
                For Each ctrl As Control In Me.grbNewInvoiceExtra.Controls
            ...
            Else
                ...
                If Not frmMain.blUseInvoiceDevice OrElse frmMain.blInvoiceDeviceOK Then Me.txbInvoiceAMT.Text = Format(IIf(dmAvailableAMT + dmReprintableAMT <= frmSelling.drInvoicePrintable("MaxInvoiceAMT"), dmAvailableAMT + dmReprintableAMT, frmSelling.drInvoicePrintable("MaxInvoiceAMT")), "#,0.00")
            End If
        End If

        If "|21|272|".IndexOf(frmMain.dtLoginStructure.Rows.Find(frmSelling.drSalesBill("StoreID").ToString)("ParentAreaID").ToString) > 0 Then '上海及测试城市
        ...
        If dmReprintableAMT > 0 Then
            Dim drPending As DataRow = frmSelling.dtInvoice.Select("ReprintableAMT>0")(0)
        ...
        If frmSelling.blInvoicePrinterMultiUser Then Me.btnGetNewInvoiceNo.Text = "10秒后获取下一个可用号码"
    End Sub
    ...
    Private Sub newInvoiceNoTimer_Tick(ByVal sender As Object, ByVal e As System.EventArgs) Handles newInvoiceNoTimer.Tick
        ...
        If bSeconds = 0 Then
            ...
            Me.btnGetNewInvoiceNo.Enabled = (frmSelling.blInvoicePrinterMultiUser AndAlso Me.btnPrintInvoice.Enabled)
            Me.newInvoiceNoTimer.Enabled = False
        ElseIf frmSelling.blInvoicePrinterMultiUser Then
    ...
    Private Sub dgvInvoice_CellContentClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvInvoice.CellContentClick
        ...
        If frmMain.dtAllowedRight.Select("RightName='SalesBillInvoiceReprint'").Length = 0 Then
        ...
        Dim drInvoice As DataRow = frmSelling.dtInvoice.Select("RowID=" & Me.dgvInvoice("RowID", e.RowIndex).Value.ToString)(0)
        If drInvoice("InvoiceState").ToString = "正常" Then
            If drInvoice("ReprintableTimes") = 0 Then
                ...
                With Me.dgvInvoice.Columns("Remarks")
                    ...
                    .AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells
                End With
            ElseIf DateAdd(DateInterval.Second, frmSelling.iDifferenceSeconds, drInvoice("ReprintableTime")) < Now() Then
            ...
            Else
                ...
                If Not frmMain.blUseInvoiceDevice OrElse frmMain.blInvoiceDeviceOK Then
                    ...
                    With Me.dgvInvoice
                        ...
                        End With
                    End With
                    If dmAvailableAMT + dmReprintableAMT >= frmSelling.drInvoicePrintable("MinInvoiceAMT") Then
                        Me.txbInvoiceAMT.Text = Format(IIf(dmAvailableAMT + dmReprintableAMT <= frmSelling.drInvoicePrintable("MaxInvoiceAMT"), dmAvailableAMT + dmReprintableAMT, frmSelling.drInvoicePrintable("MaxInvoiceAMT")), "#,0.00")
                    Else
        ...
        Else '撤销重打印
            ...
            With Me.dgvInvoice
                ...
                End With
                With .Columns("CancelledReason")
                    If frmSelling.dtInvoice.Select("Isnull(CancelledReason,'')<>''").Length = 0 Then
                        .Visible = False
                    Else
                        .AutoSizeMode = DataGridViewAutoSizeColumnMode.None
                        .AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells
                ...
                End With
                With .Columns("Remarks")
                    If frmSelling.dtInvoice.Select("Isnull(Remarks,'')<>''").Length = 0 Then
                        .Visible = False
                    Else
                        .AutoSizeMode = DataGridViewAutoSizeColumnMode.None
                        .AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells
        ...
        If Me.pnlNewInvoice.Visible Then
            Me.chbPrintLine.Visible = ("|21|272|".IndexOf(frmMain.dtLoginStructure.Rows.Find(frmSelling.drSalesBill("StoreID").ToString)("ParentAreaID").ToString) > 0) '上海及测试城市
        Else
            Me.chbPrintLine.Visible = False
        End If
    End Sub
    ...
    Private Sub txbInvoiceAMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txbInvoiceAMT.TextChanged
        ...
        ElseIf CDec(Me.txbInvoiceAMT.Text.Trim.Replace(",", "")) > dmAvailableAMT + dmReprintableAMT Then
            ...
            Me.btnPrintInvoice.Enabled = False
        ElseIf CDec(Me.txbInvoiceAMT.Text.Trim.Replace(",", "")) < frmSelling.drInvoicePrintable("MinInvoiceAMT") Then
            Me.lblError.Visible = True
            Me.lblError.Text = "（当地税务部门规定：单张发票的最小金额不能低于 " & Format(frmSelling.drInvoicePrintable("MinInvoiceAMT"), "#,0.00").Replace(".00", "") & " 元！）"
            Me.btnPrintInvoice.Enabled = False
        ElseIf CDec(Me.txbInvoiceAMT.Text.Trim.Replace(",", "")) > frmSelling.drInvoicePrintable("MaxInvoiceAMT") Then
            Me.lblError.Visible = True
            Me.lblError.Text = "（当地税务部门规定：单张发票的最大金额不能超过 " & Format(frmSelling.drInvoicePrintable("MaxInvoiceAMT"), "#,0.00").Replace(".00", "") & " 元！）"
            Me.btnPrintInvoice.Enabled = False
        Else
    ...
    Private Sub txbInvoiceCode_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles txbInvoiceCode.Leave
        ...
        If Me.txbInvoiceCode.Text = "" Then Me.txbInvoiceCode.Text = frmSelling.sInvoiceCode
    End Sub
    ...
    Private Sub txbInvoiceNo_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles txbInvoiceNo.Leave
        ...
        If Me.txbInvoiceNo.Text = "" Then Me.txbInvoiceNo.Text = frmSelling.sInvoiceNo
    End Sub
    ...
    Private Sub btnGetNewInvoiceNo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGetNewInvoiceNo.Click
        ...
        If DB.IsConnected Then
            Try
                Dim drInvoiceCodeNo As DataRow = DB.GetDataTable("Select LastInvoiceCode,LastInvoiceNo From InvoiceCodeNo Where StoreID=" & frmSelling.drSalesBill("StoreID").ToString & " And PrinterDevice='" & frmSelling.sInvoicePrinterDevice.Replace("'", "''") & "'").Rows(0)
                frmSelling.sInvoiceCode = drInvoiceCodeNo("LastInvoiceCode").ToString
                frmSelling.sInvoiceNo = drInvoiceCodeNo("LastInvoiceNo").ToString
                If frmSelling.sInvoiceNo <> "" Then
                    frmSelling.sInvoiceNo = (CLng(frmSelling.sInvoiceNo) + 1).ToString
                    If frmSelling.sInvoiceNo.Length < drInvoiceCodeNo("LastInvoiceNo").ToString.Length Then frmSelling.sInvoiceNo = StrDup(drInvoiceCodeNo("LastInvoiceNo").ToString.Length - frmSelling.sInvoiceNo.Length, "0") & frmSelling.sInvoiceNo
                    Me.txbInvoiceCode.Text = frmSelling.sInvoiceCode
                    Me.txbInvoiceNo.Text = frmSelling.sInvoiceNo
                End If
            ...
            Catch ex As Exception
                frmSelling.sInvoiceCode = "" : frmSelling.sInvoiceNo = ""
    ...
    Private Sub btnPrintInvoice_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrintInvoice.Click
        ...
        For Each drPayment As DataGridViewRow In frmSelling.dgvPayment.Rows
            If sPaymentTerm.IndexOf(drPayment.Cells("PaymentTerm").FormattedValue.ToString) = -1 Then sPaymentTerm = sPaymentTerm & drPayment.Cells("PaymentTerm").FormattedValue.ToString.Replace("_后付", "").Replace("＋保证金", "") & "/"
        Next
        sPaymentTerm = sPaymentTerm.Substring(0, sPaymentTerm.Length - 1)
        ...
        Dim sSQL As String = "Exec PrintInvoice @SalesBillID=" & frmSelling.sSalesBillID & ",@TotalPaymentAMT=" & CDec(Me.txbTotalPaymentAMT.Text).ToString & ",@TotalPrintedInvoiceAMT=" & CDec(Me.txbTotalPrintedInvoiceAMT.Text).ToString & ",@RowID=" & (Me.dgvInvoice.RowCount + 1).ToString & ","
        If Not frmMain.blUseInvoiceDevice Then sSQL = sSQL & "@PrinterDevice='" & frmSelling.sInvoicePrinterDevice.Replace("'", "''") & "',@InvoiceCode='" & sInvoiceCode & "',@InvoiceNo='" & sInvoiceNo & "',"
        ...
        If sPaymentTerm <> "现金" Then sSQL = sSQL & "@PaymentTerm='" & sPaymentTerm & "',"

        Dim dtNew As DataTable = frmSelling.dtInvoice.Copy, drNew As DataRow, drInvoice As DataRow, dtCancelled As DataTable = Nothing, drCancelled As DataRow
        dtNew.Rows.Clear()
        ...
        If dmReprintableAMT > 0 Then
            ...
            dtCancelled.Columns.Add("SalesBillID", System.Type.GetType("System.Int32"))
            ...
            For Each drInvoice In frmSelling.dtInvoice.Select("ReprintableAMT>0", "RowID")
                If DateAdd(DateInterval.Second, frmSelling.iDifferenceSeconds, drInvoice("ReprintableTime")) < Now() Then
                ...
                ElseIf dmBalanceAMT > 0 Then
                    ...
                    drCancelled("SalesBillID") = frmSelling.sSalesBillID
        ...
        If dtCancelled IsNot Nothing AndAlso dtCancelled.Rows.Count > 0 Then
            If DB.ModifyTable("Select SalesBillID,RowID,CancelledReason,ReprintableAMT Into #CancelledInvoice From SalesBillInvoice Where 1=2") = -1 OrElse DB.BulkCopyTable("#CancelledInvoice", dtCancelled) = -1 Then
                Me.ResetInterface()
        ...
        ElseIf dtResult.Rows(0)("Result").ToString <> "OK" Then
            Select Case dtResult.Rows(0)("Result").ToString
                ...
                Case "InvoiceNoUsed"
                    bSeconds = 10
                    frmSelling.blInvoicePrinterMultiUser = True
                    Me.btnGetNewInvoiceNo.Text = "10秒后获取下一个可用号码"
                    Me.btnGetNewInvoiceNo.Enabled = False
                    Me.newInvoiceNoTimer.Enabled = True

                    frmSelling.sInvoiceCode = dtResult.Rows(0)("LastInvoiceCode").ToString
                    frmSelling.sInvoiceNo = dtResult.Rows(0)("LastInvoiceNo").ToString
                    frmSelling.sInvoiceNo = (CLng(frmSelling.sInvoiceNo) + 1).ToString
                    If frmSelling.sInvoiceNo.Length < dtResult.Rows(0)("LastInvoiceNo").ToString.Length Then frmSelling.sInvoiceNo = StrDup(dtResult.Rows(0)("LastInvoiceNo").ToString.Length - frmSelling.sInvoiceNo.Length, "0") & frmSelling.sInvoiceNo
                    MessageBox.Show("发票代码""" & sInvoiceCode & """中的发票号码""" & sInvoiceNo & """已被使用！    " & Chr(13) & Chr(13) & _
                                    "下面是系统自动获取的下一个可用的发票代码及发票号码：    " & Chr(13) & Chr(13) & _
                                    "发票代码：" & frmSelling.sInvoiceCode & Space(4) & Chr(13) & _
                                    "发票号码：" & frmSelling.sInvoiceNo & Space(4) & Chr(13) & Chr(13) & _
            ...
            End Select
        Else
            Dim sRestoreSQL As String = "Exec PrintInvoiceRestore @StoreID=" & frmSelling.drSalesBill("StoreID").ToString & ",@SalesBillID=" & frmSelling.drSalesBill("SalesBillID").ToString & ",@RowID=" & drNew("RowID").ToString & ","
            If Not frmMain.blUseInvoiceDevice Then sRestoreSQL = sRestoreSQL & "@PrinterDevice='" & frmSelling.sInvoicePrinterDevice.Replace("'", "''") & "',"
            ...
            If frmMain.blUseInvoiceDevice Then '如果需要开票器 AndAlso Not My.Settings.IsInTraining
                            ...
                            If drInvoice("InvoiceCode").ToString <> "" AndAlso drInvoice("InvoiceNo").ToString <> "" Then
                                ...
                                If sResult.IndexOf("0") = 0 Then
                                    ...
                                    With dtPrintItem.Rows
                                        ...
                                        .Add(10, "家乐福小票号：" & frmSelling.lblSalesBillCode.Text.Replace(" ", "") & IIf(drInvoice("RowID") = 1 AndAlso drInvoice("InvoiceAMT") = dmTotalPayment, "", "-" & drInvoice("RowID").ToString)) '家乐福小票号（含标题）
                                        .Add(11, "") '购买统计信息（含标题）
                                        .Add(12, "") '折扣金额
                ...
                Else
                    Try
                        DB.ModifyTable("Update SalesBillInvoice Set InvoiceCode='" & sInvoiceCode & "',InvoiceNo='" & sInvoiceNo & "' Where SalesBillID=" & frmSelling.drSalesBill("SalesBillID").ToString & " And RowID=" & drNew("RowID").ToString)
                    Catch
                    End Try
            ...
            With dtPrintItem.Rows
                ...
                .Add(10, "家乐福小票号：" & frmSelling.lblSalesBillCode.Text.Replace(" ", "") & IIf(Me.dgvInvoice.RowCount = 0 AndAlso dmInvoiceAMT = dmTotalPayment, "", "-" & drNew("RowID").ToString)) '家乐福小票号（含标题）
            ...
            If sResult = "OK" Then
                If Not frmMain.blUseInvoiceDevice Then
                    frmSelling.sInvoiceCode = sInvoiceCode
                    frmSelling.sInvoiceNo = sInvoiceNo
                    frmSelling.sInvoiceNo = (CLng(frmSelling.sInvoiceNo) + 1).ToString
                    If frmSelling.sInvoiceNo.Length < sInvoiceNo.Length Then frmSelling.sInvoiceNo = StrDup(sInvoiceNo.Length - frmSelling.sInvoiceNo.Length, "0") & frmSelling.sInvoiceNo
                    Me.txbInvoiceNo.Text = frmSelling.sInvoiceNo
                    bSeconds = 10
                    If frmSelling.blInvoicePrinterMultiUser Then
                        Me.btnGetNewInvoiceNo.Text = "10秒后获取下一个可用号码"
                        Me.btnGetNewInvoiceNo.Enabled = False
                    End If
                    Me.newInvoiceNoTimer.Enabled = True
            ...
            End If
            drNew.AcceptChanges()
            frmSelling.dtInvoice.Merge(dtNew.Copy)
            ...
            If dtCancelled IsNot Nothing AndAlso dtCancelled.Rows.Count > 0 Then
                For Each drCancelled In dtCancelled.Rows
                    drInvoice = frmSelling.dtInvoice.Select("RowID=" & drCancelled("RowID").ToString)(0)
            ...
            With Me.dgvInvoice.Columns("SourceInvoice")
                If frmSelling.dtInvoice.Select("Isnull(SourceInvoice,'')<>''").Length = 0 Then
                    .Visible = False
                Else
                    .Visible = True
                    .AutoSizeMode = DataGridViewAutoSizeColumnMode.None
        ...
        If blNeedResetInfo Then
            ...
            ElseIf dmAvailableAMT + dmReprintableAMT > 0 Then
                If Me.grbPrintedInvoice.Visible Then Me.dgvInvoice.Select()
                If Me.chbMultiPrint.Visible = False OrElse Me.chbMultiPrint.Checked = False Then Me.txbInvoiceAMT.Text = Format(IIf(dmAvailableAMT + dmReprintableAMT <= frmSelling.drInvoicePrintable("MaxInvoiceAMT"), dmAvailableAMT + dmReprintableAMT, frmSelling.drInvoicePrintable("MaxInvoiceAMT")), "0.00")
                Me.txbInvoiceAMT.Select()
                Me.txbInvoiceAMT.SelectAll()
    ...
    Private Sub btnPrintInvoice_EnabledChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrintInvoice.EnabledChanged
        Me.btnGetNewInvoiceNo.Enabled = (frmSelling.blInvoicePrinterMultiUser AndAlso bSeconds = 0 AndAlso Me.btnPrintInvoice.Enabled)
        If Me.btnPrintInvoice.Enabled AndAlso dmTotalRebateAMT > 0 Then
            If frmSelling.drSalesBill("InvoicePrintDiscount").ToString = "" Then
                Me.chbPrintDiscount.Visible = True
            Else
                Me.chbPrintDiscount.Checked = frmSelling.drSalesBill("InvoicePrintDiscount")
    ...
    Private Sub ResetInterface()
        ...
        Else
            ...
            Me.dgvInvoice.Columns("InvoiceCode").Visible = (frmSelling.dtInvoice.Select("Isnull(InvoiceCode,'')<>''").Length > 0)
            Me.dgvInvoice.Columns("InvoiceNo").Visible = (frmSelling.dtInvoice.Select("Isnull(InvoiceNo,'')<>''").Length > 0)
    ...
    Private Function FinalPrinting(ByVal sRowID As String) As String
        ...
        Try
            ...
            Invoice.DocumentName = "Invoice_" & frmSelling.lblSalesBillCode.Text.Replace(" ", "") & "-" & sRowID
            If "|21|272|".IndexOf("|" & frmMain.dtLoginStructure.Rows.Find(frmSelling.drSalesBill("StoreID").ToString)("ParentAreaID").ToString & "|") > -1 Then '上海''''
                ...
                'ElseIf "|24|25|".IndexOf("|" & frmMain.dtLoginStructure.Rows.Find(frmSelling.drSalesBill("StoreID").ToString)("ParentAreaID").ToString & "|") > -1 Then '重庆/成都/测试城市272|''''''
            ElseIf "|24|25|304|".IndexOf("|" & frmMain.dtLoginStructure.Rows.Find(frmSelling.drSalesBill("StoreID").ToString)("ParentAreaID").ToString & "|") > -1 Then '重庆/成都/资阳/测试城市272|''''''
                'modify code 041:end-------------------------------------------------------------------------
                AddHandler Invoice.PrintPage, AddressOf InvoiceCQ_PrintPage
            ElseIf "|27|".IndexOf("|" & frmMain.dtLoginStructure.Rows.Find(frmSelling.drSalesBill("StoreID").ToString)("ParentAreaID").ToString & "|") > -1 Then '武汉
    ...
    Private Sub Invoice_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs)
        ...
        For Each drItem As DataRow In frmSelling.dtInvoiceLayout.Select("IsVisible=1 And ItemID<11")
        ...
        drawFont = New Font("宋体", frmSelling.dtInvoiceLayout.Rows(5)("FontSize"))
        Dim x As Integer = frmSelling.dtInvoiceLayout.Rows(4)("X"), y As Integer = frmSelling.dtInvoiceLayout.Rows(4)("Y") '发票金额的坐标
        y += 5
    ...
    Private Sub InvoiceSHWithLine_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs)
        ...
        drawFormat.FormatFlags = StringFormatFlags.NoWrap
        Dim x As Integer = frmSelling.dtInvoiceLayout.Rows(0)("X"), y As Integer = frmSelling.dtInvoiceLayout.Rows(0)("Y")
        ...
        If My.Settings.IsInTraining Then
            ...
            x = frmSelling.dtInvoiceLayout.Rows(0)("X") : y = frmSelling.dtInvoiceLayout.Rows(0)("Y")
        End If
        ...
        x += 168
        drawFormat.Alignment = StringAlignment.Far
        e.Graphics.DrawString("开票门店：" & frmMain.dtLoginStructure.Rows.Find(frmSelling.drSalesBill("StoreID").ToString)("AreaName").ToString.Substring(4) & "    开票人：" & frmMain.sLoginUserRealName & "    收款人：" & frmSelling.drSalesBill("CreatorName").ToString, drawFont, drawBrush, x, y, drawFormat)
    ...
    Private Sub InvoiceSHWithoutLine_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs)
        ...
        If sInvoiceSecurityCode = "" Then
            e.Graphics.DrawString("开票门店：" & frmMain.dtLoginStructure.Rows.Find(frmSelling.drSalesBill("StoreID").ToString)("AreaName").ToString.Substring(4) & "    发票代码：" & sInvoiceCode & "    发票号码：" & sInvoiceNo, drawFont, drawBrush, x, y, drawFormat)
        Else
            e.Graphics.DrawString("开票门店：" & frmMain.dtLoginStructure.Rows.Find(frmSelling.drSalesBill("StoreID").ToString)("AreaName").ToString.Substring(4) & "    发票代码：" & sInvoiceCode & "    发票号码：" & sInvoiceNo & "    防伪码：" & sInvoiceSecurityCode, drawFont, drawBrush, x, y, drawFormat)
        End If
        y += 8 '开票人
        e.Graphics.DrawString("开票人：" & frmMain.sLoginUserRealName, drawFont, drawBrush, x, y, drawFormat)
        x += 35 '收款人
        e.Graphics.DrawString("收款人：" & frmSelling.drSalesBill("CreatorName").ToString, drawFont, drawBrush, x, y, drawFormat)
        x += 35 '付款方式
        e.Graphics.DrawString("付款方式：" & dtPrintItem.Rows.Find(7)("PrintText").ToString, drawFont, drawBrush, x, y, drawFormat)
        x += 35 '标题“开票单位（盖章）”
        e.Graphics.DrawString("开票单位（盖章）：", drawFont, drawBrush, x, y, drawFormat)
